using System;
using System.Collections.Generic;

namespace AutocountAPI.Models;

public partial class ColumnLock
{
    public string DocType { get; set; } = null!;

    public string DtlType { get; set; } = null!;

    public string Indent { get; set; } = null!;

    public string FontStyle { get; set; } = null!;

    public string MainItem { get; set; } = null!;

    public string Numbering { get; set; } = null!;

    public string Location { get; set; } = null!;

    public string BatchNo { get; set; } = null!;

    public string Description { get; set; } = null!;

    public string FurtherDescription { get; set; } = null!;

    public string YourPono { get; set; } = null!;

    public string YourPodate { get; set; } = null!;

    public string YourDono { get; set; } = null!;

    public string YourDodate { get; set; } = null!;

    public string OurPono { get; set; } = null!;

    public string OurPodate { get; set; } = null!;

    public string DeliveryDate { get; set; } = null!;

    public string PostToStockDate { get; set; } = null!;

    public string ProjNo { get; set; } = null!;

    public string DeptNo { get; set; } = null!;

    public string Qty { get; set; } = null!;

    public string Focqty { get; set; } = null!;

    public string UnitPrice { get; set; } = null!;

    public string Discount { get; set; } = null!;

    public string TaxCode { get; set; } = null!;

    public string Tax { get; set; } = null!;

    public string SubTotal { get; set; } = null!;

    public string BonusPoint { get; set; } = null!;

    public string PrintOut { get; set; } = null!;

    public string AccNo { get; set; } = null!;

    public string ForeignCharges { get; set; } = null!;

    public string LocalCharges { get; set; } = null!;

    public string Duty { get; set; } = null!;

    public string EstimatedDeliveryDate { get; set; } = null!;

    public string TaxRate { get; set; } = null!;
}
